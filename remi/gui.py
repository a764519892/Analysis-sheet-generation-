...
class GenericDialog_1(Container):
    ...
    def __init__(self, title='', message='', *args, **kwargs):
        ...
        Args:
            ...
            kwargs: See Container.__init__()
        """
        super(GenericDialog_1, self).__init__(*args, **kwargs)
        ...
        if len(message) > 0:
            ...
            label0.css_color = "rgb(255,75,228)"
            label0.css_flex_direction = "column"
            label0.css_font_size = "29px"
            label0.css_font_style = "italic"

            label0.css_left = "46.5%"


            label0.css_margin = '20px'
            label0.css_position = "absolute"

            label0.text = message
            label0.variable_name = "label0"
            self.append(label0, 'message')
            # m = Label(message)
            # m.css_margin = '20px'
            # m.css_left='300px'
    ...
    def add_field_with_image(self, key, label_description):
        image0 = Image()
        image0.attr_editor_newclass = False
        image0.attr_src = load_resource(label_description) #gui.load_resource() 添加图片地址  #'/res:来样分析表.png'
        image0.css_height = "100%"
        #image0.css_left = "20px"
        #image0.css_margin = "0px"
        #image0.css_position = "absolute"
        #image0.css_top = "20px"
        image0.css_width = "100.0%"
        image0.variable_name = "image0"
        container0 = Container()
        container0.attr_class = "Container"
        container0.attr_editor_newclass = False
        container0.css_height = "1584px"
        #container0.css_left = "5.0%"
        #container0.css_right = "5.0%"
        container0.css_position = "absolute"
        #container0.css_top = "10%"
        container0.css_width = "1224px"

        container0.variable_name = "container0"
        container0.append(image0, key='lbl' + key)
        #container.append(self.inputs[key], key=key)
        self.container.append(container0, key=key)
    def add_field_with_button(self, key, label_description, field,dizhi,chang):
        container = HBox()
        ...
        for i in range(4):
            ...
            button0.css_margin = "10px"
            button0.css_left = "20px"
            button0.css_order = "495947991248"
            button0.css_position = "static"
            button0.css_top = "20px"
            button0.css_width = "100px"
            button0.text = label_description
            button0.variable_name = "button1"

            container.append(button0, key='lbl' + 'button{}'.format(i))
            container.children['lbl'+'button{}'.format(i)].onclick.do(self.onclick_button, dizhi[i])
            container.append(self.inputs[key], key=key)
            self.container.append(container, key=key)
        # for i in range(chang):
        #     container.children['button{}'.format(i)].onclick.do(self.onclick_button, dizhi[i])
...
class GenericDialog(Container):
    ...
    def add_field_with_label(self, key, label_description, field):
        ...
        image0 = Image()
        image0.attr_editor_newclass = False
        image0.attr_src = label_description
        image0.css_height = "300.0px"
        image0.css_left = "20px"
        image0.css_margin = "0px"
        image0.css_position = "absolute"
        image0.css_top = "20px"
        image0.css_width = "200.0px"
        image0.variable_name = "image0"
        container = HBox()
        container.style.update({'justify-content': 'space-between', 'overflow': 'auto', 'padding': '3px'})
        container.append(image0, key='lbl' + key)
        container.append(self.inputs[key], key=key)
        self.container.append(container, key=key)
    def add_field_with_button_duo(self, key, label_description,shu,dizhi,gg):
        container = HBox()
        container.style.update({'justify-content': 'space-between', 'overflow': 'auto', 'padding': '3px'})
        for i in range(4):
            #self.inputs[key] = field
            ...
            button0.css_top = "20px"
            button0.css_width = "100px"
            button0.text = label_description[i]
            button0.variable_name = "button1"
            container.append(button0, key='lbl{}'.format(i) + key)
            container.children['lbl{}'.format(i)+key].onclick.do(self.onclick_button,dizhi[i],label_description[i])
            # button1 = MenuItem()
            # button1.attr_editor_newclass = False
            # button1.css_height = "30px"
            # button1.css_margin = "10px"
            # button1.css_left = "20px"
            # button1.css_order = "495947991248"
            # button1.css_position = "static"
            # button1.css_top = "20px"
            # button1.css_width = "100px"
            # button1.text = label_description[i+1]
            # button1.variable_name = "button1"
            # container.append(button1, key='lbl1' + key)
            # container.children['lbl1' + key].onclick.do(self.onclick_button, dizhi[i+1])

            #container.append(self.inputs[key], key=key)
        self.container.append(container, key=key)
        # for i in range(shu):
        #     self.container.children['container'].children['lbl'+'button{}'.format(i)].onclick.do(self.onclick_button,dizhi[i])
    def add_field_with_button_dan(self, key, label_description,shu,dizhi):
        container = HBox()
        container.style.update({'justify-content': 'space-between', 'overflow': 'auto', 'padding': '3px'})
        for i in range(shu):
            #self.inputs[key] = field
        ...
        # for i in range(shu):
        #     self.container.children['container'].children['lbl'+'button{}'.format(i)].onclick.do(self.onclick_button,dizhi[i])
    def onclick_button(self,emble,url,xinxi):
        #总接口调用
        if xinxi =='预告片':
            self.children['message'].text ="正在观看：" +xinxi
        elif xinxi =='预告1':
            self.children['message'].text = "正在观看：" + xinxi
        else:
            self.children['message'].text = "正在观看第{}集".format(xinxi)
        # self.children['message'].css_margin = '20px'
        # self.children['message'].css_left = '300px'
        with open('1.txt','r',encoding='utf-8') as f:   #接口判断
            jiekou=f.read()
            #print(jiekou)
            if jiekou =='播放接口1':

                dizhi='http://api.yueliangjx.com/?url=' +url
                self.container.children['video'].children['video'].attributes['src'] = dizhi
            elif jiekou =='播放接口2':

                dizhi='http://www.asys.vip/jx/?url=' +url
                self.container.children['video'].children['video'].attributes['src'] = dizhi
            elif jiekou == '播放接口3':

                dizhi = 'https://jx.elwtc.com/vip/?url=' + url
                self.container.children['video'].children['video'].attributes['src'] = dizhi
            elif jiekou == '播放接口4':

                dizhi = 'https://www.ckmov.vip/api.php?url=' + url
                self.container.children['video'].children['video'].attributes['src'] = dizhi
        #视频网站
        #self.container.children['video'].children['video'].attributes['src'] = dizhi
        #print(self.container.children['video'].children['video'].attributes['src'])
    def add_field_with_xuexi(self,key,mingcheng,dizhi):
        container = HBox()
        container.style.update({'justify-content': 'space-between', 'overflow': 'auto', 'padding': '3px'})
        for i in range(len(mingcheng)):
            ...
            button0.css_top = "20px"
            button0.css_width = "100px"
            button0.text = mingcheng[i]
            button0.variable_name = "button1"

            container.append(button0, key='lbl' + 'button{}'.format(i))
            container.children['lbl' + 'button{}'.format(i)].onclick.do(self.xuexi_click_button, dizhi[i],mingcheng[i])
            #container.append(self.inputs[key], key=key)
        self.container.append(container, key=key)
    ...
    def hide(self):
        self._base_app_instance.set_root_widget(self._old_root_widget)
    def xuexi_click_button(self, emble, url, xinxi):
        self.hide()

        self.dialog = GenericDialog_1(title='欢迎观看', message='',
                                    width='1224px')
        if xinxi =='织物简称':
            print(xinxi)
            self.dialog.add_field_with_image('image', url)
        elif xinxi =='纺纱方式':
            self.dialog.add_field_with_image('image', url)
...
class VideoPlayer(Widget):
    ...
    def __init__(self, video='', poster=None, autoplay=False, loop=False, *args, **kwargs):
        ...
        #self.attributes['screenshot']='false'
        #self.attributes['hotkey']='true'
